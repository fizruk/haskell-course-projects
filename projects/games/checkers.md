...
Проект должен быть разбит логически на модули.

Каждая часть проекта должна быть реализована таким образом,
чтобы любую другую часть можно было легко исключить из проекта. Например,
модуль для работы с базой данных должен уметь работать как на клиентской стороне,
так и на серверной.
